...
export async function generatePDF(priceOffer: PriceOffer, userId: string) {
  ...
  doc.text(`תאריך: ${new Date(priceOffer.date).toLocaleDateString("he-IL")}`, 190, 50, {
    align: "right",
  });
  doc.text(`בתוקף עד: ${new Date(priceOffer.validUntil).toLocaleDateString("he-IL")}`, 190, 60, {
    align: "right",
  });
  ...
  doc.text(`סה"כ כולל מע"מ: ₪${priceOffer.total.toFixed(2)}`, 190, finalY + 20, {
    align: "right",
  });
...
export const getPdfUrl = async (fileId: string): Promise<string | null> => {
  ...
  try {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("documents").getPublicUrl(fileRecord.file_path);
